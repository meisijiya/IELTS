---
id: T-054
goal: Task 2 essay — sub-category 5.2 (slug 33-28-health) — first essay covering this previously-uncovered sub-category from 作文真题储备.
files:
  - docs/writing/task2/33-28-health.html
deps: [T-018]
ac:
...
evidence: file exists at docs/writing/task2/33-28-health.html; bash scripts/verify-stage1b.sh docs/writing/task2/33-28-health.html exits 0; word count in [270, 290]; `<article>` carries valid `data-type` from the 8-chip whitelist; theme = tech.
size: S
...
Task 2 essay HTML page covering sub-category **5.2** (theme: tech) from `作文真题储备（近五年）_可修改.docx`. This is the **first** essay for this sub-category (not covered by Stage 1a).
...
8. Self-run `bash scripts/verify-stage1b.sh docs/writing/task2/33-28-health.html` → must exit 0.
9. Commit + push.

## Acceptance criteria

- [ ] File `docs/writing/task2/33-28-health.html` exists.
...
- [ ] `bash scripts/verify-stage1b.sh docs/writing/task2/33-28-health.html` exits 0.
- [ ] `grep -E 'data-task="task2"' docs/writing/task2/33-28-health.html` returns 1.

## Files in scope

- `docs/writing/task2/33-28-health.html` (create).
...
If the prompt under sub-category 5.2 is genuinely ambiguous (no clear question structure), mark `blocked`, write reason, escalate. Do NOT fabricate a prompt.
